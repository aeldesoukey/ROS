...
int main(int argc, char **argv){
  ...
  ros::Publisher goal_reached_pub = n.advertise<std_msgs::Time>("goal_reached",10);
  ...
  while (ros::ok())
  {
    ...
    if(rrt->goal_received)//TODO: always on rrt
    {
      ...
      std_msgs::Time goal_reached_msg;
      goal_reached_msg.data = ros::Time::now();
